...
public class ViewRatingController
{
  ...
  public void init(ViewHandler viewHandler, ViewModelFactory viewModelFactory)
  {
    ...
    otherInformationLabel.textProperty()
        .bind(viewRatingViewModel.getOtherInformation());

    displayRatings(viewRatingViewModel
        .getAllRatingsOnMember(viewRatingViewModel.getMemberUsername()));
  }
  /**
  ...
  {
    if (userNameLabel.textProperty().getValue()
        .equals(viewRatingViewModel.getUserType()))
    {
  ...
  public void displayRatings(ArrayList<Rating> ratings)
  {
    if (ratings != null && !ratings.isEmpty())
    {
      for (int i = 0; i < ratings.size(); i++)
      {
        ...
        ratingBox.setStyle(
            "-fx-border-color: #FF8C64; -fx-border-width: 3; -fx-background-color: #FF8C64;");
